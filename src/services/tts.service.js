...
const playAudio = (filePath) => new Promise((resolve, reject) => {
  ...
  switch (platform) {
    case 'darwin':
      ...
      break;
    case 'win32':
      command = `powershell -Command "Add-Type -AssemblyName PresentationCore; $mp = New-Object System.Windows.Media.MediaPlayer; $mp.Open([Uri]'${filePath}'); $mp.Play(); Start-Sleep -Seconds 1; while ($mp.NaturalDuration.HasTimeSpan -and $mp.Position -lt $mp.NaturalDuration.TimeSpan) { Start-Sleep -Milliseconds 200 }; $mp.Stop(); exit 0"`;
      break;
    case 'linux':
