...
const emitStaffDisplayUpdateForCounters = async (
  counterIds,
  reason,
  extra = {},
) => {
  ...
  const uniqueCounterIds = [
    ...new Set(counterIds.map((counterId) => String(counterId))),
  ];
  ...
  await Promise.all(
    uniqueCounterIds.map(async (counterId) => {
      try {
        ...
        console.error(
          `Không thể cập nhật staff display cho phòng ${counterId}: ${error.message}`,
        );
      }
    }),
  );
...
const emitTicketCalled = async (ticket, counter, reason = "ticket-called") => {
  ...
  const formattedNumber = buildTicketPresentation(
    ticket,
    counter,
  ).formattedNumber;
