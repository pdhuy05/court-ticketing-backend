  ...
  return recallTickets.map((ticket) => {
    ...
    return {
      ...
      waitingMinutes: ticket.recalledAt
        ? Math.max(
            Math.floor(
              (now - new Date(ticket.recalledAt).getTime()) / (1000 * 60),
            ),
            0,
          )
        : 0,
...
const getCounterDisplay = async (counterId) => {
  ...
  const activeServiceRelations = serviceRelations.filter(
    (r) => r.serviceId?.isActive === true,
  );
  const serviceIds = activeServiceRelations.map(
  ...
  return {
    ...
    currentTicket: processingTickets.length
      ? formatTicket(processingTickets[0])
      : null,
...
const getCurrentTicketForStaff = async (
  counterId,
  staffId,
  allowedServiceIds,
) => {
...
const getMyCounter = async (counterId, staffId = null) => {
  const counter = await Counter.findById(counterId);
  if (!counter) {
    throw new ApiError(404, "Không tìm thấy phòng của bạn");
  }
...
const getStaffDisplay = async (counterId, staffId = null) => {
  ...
  const currentTicket = staffId
    ? processingTickets.find((t) => String(t.staffId) === String(staffId)) ||
      null
