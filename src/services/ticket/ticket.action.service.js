...
const canAccessService = (serviceIds, serviceId) =>
  serviceIds.includes(String(serviceId));
...
const ensureNoProcessingTicket = async (counterId, staffId = null) => {
  ...
  if (existingProcessing) {
    ...
    if (staffId) {
      throw new ApiError(
        400,
        `Bạn đang xử lý vé ${presentation.formattedNumber}. Vui lòng hoàn thành hoặc bỏ qua vé hiện tại trước`,
      );
    }
...
const getAvailableCounterRelationsForOnDutyStaff = async (
  serviceId,
  relations,
) => {
  ...
  return relations.filter((relation) =>
    onDutyCounterIdSet.has(
      String(relation.counterId?._id || relation.counterId),
    ),
  );
};
...
const resolveIssueCounter = async (serviceId, requestedCounterId = null) => {
  ...
  if (requestedCounterId) {
    const matchedRelation = activeRelations.find(
      (relation) =>
        String(relation.counterId?._id || relation.counterId) ===
        String(requestedCounterId),
    );

    if (!matchedRelation?.counterId?.isActive) {
      throw new ApiError(
        400,
        "phòng được chọn không phục vụ quầy này hoặc đã bị khóa",
      );
    }
...
const createTicket = async ({ serviceId, name, phone, counterId = null }) => {
  ...
  if (!service.isActive) {
    throw new ApiError(
      400,
      `quầy ${service.name} đã bị vô hiệu hóa, không thể lấy số`,
    );
  }
  ...
  if (!service.isOpen) {
    throw new ApiError(
      400,
      `quầy ${service.name} hiện đang tạm đóng. Vui lòng quay lại sau.`,
    );
  }

  const { issueCounter, availableCounters } = await resolveIssueCounter(
    serviceId,
    counterId,
  );
  ...
  const servicePrefix =
    typeof service.prefixNumber === "number" ? service.prefixNumber : 0;
  ...
  console.log(`\x1b[36m│\x1b[0m  quầy : \x1b[1m${service.name}\x1b[0m`);
  ...
  return {
    ...
    availableCounters: availableCounters.map(
      (availableCounter) => availableCounter.counterId,
    ),
    qrData,
  };
};
...
const callNext = async (counterId, staffId = null) => {
  ...
  while (!nextTicket) {
    if (retries++ >= MAX_RETRIES) {
      throw new ApiError(
        409,
        "Không thể gọi số do xung đột dữ liệu, vui lòng thử lại",
      );
    }
    ...
    if (!candidateTicket) {
      throw new ApiError(
        404,
        "Không có ticket đang chờ trong danh sách quầy được phân quyền",
      );
    }

    if (staffId) {
      try {
        await assertStaffCanHandleService(
          staffId,
          counterId,
          candidateTicket.serviceId._id,
        );
      } catch (error) {
        if (
...
const callById = async (ticketId, counterId, staffId = null) => {
  ...
  if (ticket.status !== TicketStatus.WAITING) {
    throw new ApiError(
      400,
      `Ticket đang ở trạng thái ${ticket.status}, không thể gọi. Chỉ có thể gọi ticket đang chờ`,
    );
  }
  ...
  if (
    !canAccessService(
      ...
      ticket.serviceId?._id || ticket.serviceId,
    )
  ) {
    const serviceName = ticket.serviceId?.name || "không xác định";
    throw new ApiError(403, `Bạn không có quyền xử lý quầy ${serviceName}`);
  }
  ...
  const belongsToCounter = ticket.isRecall
    ...
    : String(ticket.queueCounterId?._id || ticket.queueCounterId || "") ===
      String(counterId);

  if (!belongsToCounter) {
    throw new ApiError(
      403,
      `Ticket không thuộc danh sách xử lý của phòng ${counter.name}`,
    );
  }
...
const recallTicket = async (ticketId, counterId, staffId = null) => {
  ...
  if (ticket.status !== TicketStatus.WAITING) {
    throw new ApiError(
      400,
      `Ticket đang ở trạng thái ${ticket.status}, không thể gọi lại. Chỉ gọi lại được ticket đang chờ`,
    );
  }
  ...
  if (
    ...
    throw new ApiError(
      400,
      `Ticket không nằm trong danh sách gọi lại của phòng ${counter.name}`,
    );
  }
...
const recallProcessingTicket = async (ticketId, counterId, staffId = null) => {
  ...
  if (ticket.status !== TicketStatus.PROCESSING) {
    throw new ApiError(
      400,
      `Ticket đang ở trạng thái ${ticket.status}, không thể gọi lại. Chỉ gọi lại được ticket đang xử lý`,
    );
  }

  if (String(ticket.counterId) !== String(counterId)) {
    throw new ApiError(403, `Ticket không thuộc phòng ${counter.name}`);
  }
  ...
  if (staffId && ticket.staffId && String(ticket.staffId) !== String(staffId)) {
    throw new ApiError(
      403,
      "Bạn chỉ được phép gọi lại ticket đang xử lý của chính mình",
    );
  }
...
const cancelRecallTicket = async (
  ticketId,
  counterId,
  staffId = null,
  reason = "",
  ...
  if (ticket.status !== TicketStatus.WAITING) {
    throw new ApiError(
      400,
      `Ticket đang ở trạng thái ${ticket.status}, không thể hủy gọi lại. Chỉ hủy được ticket đang chờ`,
    );
  }
  ...
  if (
    ...
    throw new ApiError(
      400,
      "Ticket không nằm trong danh sách gọi lại của phòng hiện tại",
    );
  }
...
const completeTicket = async (ticketId, counterId = null, staffId = null) => {
  ...
  if (ticket.status !== TicketStatus.PROCESSING) {
    throw new ApiError(
      400,
      `Ticket đang ở trạng thái ${ticket.status}, không thể hoàn thành. Chỉ hoàn thành được ticket đang xử lý`,
    );
  }
  ...
  if (counterId && String(ticket.counterId) !== String(counterId)) {
    throw new ApiError(
      403,
      "Bạn chỉ được phép hoàn thành ticket của phòng được gán",
    );
  }
...
const skipTicket = async (ticketId, reason = "", counterId, staffId = null) => {
  ...
  if (ticket.status !== TicketStatus.PROCESSING) {
    throw new ApiError(
      400,
      `Ticket đang ở trạng thái ${ticket.status}, không thể bỏ qua. Chỉ bỏ qua được ticket đang xử lý`,
    );
  }
  ...
  if (counterId && String(ticket.counterId) !== String(counterId)) {
    throw new ApiError(
      403,
      "Bạn chỉ được phép bỏ qua ticket của phòng được gán",
    );
  }
...
const backToWaiting = async (
  ...
  if (ticket.status !== TicketStatus.PROCESSING) {
    throw new ApiError(
      400,
      `Ticket đang ở trạng thái ${ticket.status}, không thể trả về hàng chờ. Chỉ áp dụng với ticket đang xử lý`,
    );
  }
