...
const askPublicAssistantSchema = Joi.object({
...
const updatePublicKnowledgeSchema = Joi.object({
  knowledge: Joi.string().trim().max(50000).allow("").required().messages({
    "string.max": "Nội dung dữ liệu không được quá 50000 ký tự",
  }),
});

module.exports = { askPublicAssistantSchema, updatePublicKnowledgeSchema };