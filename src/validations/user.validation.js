...
const validatePasswordStrength = (value, helpers) => {
  const username = String(
    helpers.state.ancestors?.[0]?.username || "",
  ).toLowerCase();
  ...
  if (
    WEAK_PASSWORD_PATTERNS.some((pattern) =>
...
const createStaffSchema = Joi.object({
  username: Joi.string()
    ...
    .messages({
      ...
      "string.pattern.base":
        "Tên đăng nhập chỉ được chứa chữ cái, số, dấu chấm và gạch dưới",
    }),
  password: Joi.string()
    .trim()
    .min(8)
    ...
    .messages({
      ...
      "password.tooWeak":
        'Mật khẩu quá yếu, không được chứa các chuỗi dễ đoán như "123456" hoặc "password"',
    }),
...
const assignStaffServicesSchema = Joi.object({
  serviceIds: Joi.array().items(objectId).required().unique().messages({
