...
const getDuplicateFieldLabel = (err) => {
  ...
  if (duplicatedFields.length === 0) return null;
  const compoundKey = duplicatedFields.join("+");
  if (duplicateFieldLabelMap[compoundKey]) return duplicateFieldLabelMap[compoundKey];
  if (duplicatedFields.length === 1) return duplicateFieldLabelMap[duplicatedFields[0]] || null;
  return null;
};

const normalizeError = (err) => {
  if (err instanceof ApiError || err?.statusCode) return err;
  ...
  if (err?.name === "CastError") return new ApiError(400, `${err.path || "Dữ liệu"} không hợp lệ`);
  if (err?.code === 11000) {
    const fieldLabel = getDuplicateFieldLabel(err);
    if (fieldLabel) return new ApiError(409, `${fieldLabel} đã tồn tại, vui lòng chọn giá trị khác`);
...
app.use((err, req, res, next) => {
  ...
  notifySystemError({
    ...
    meta:    { statusCode: normalizedError.statusCode || 500 },
  });
